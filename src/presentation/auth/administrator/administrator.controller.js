...
class AuthAdministratorController {
    constructor(authAdministratorRepository) {
        ...
        this.updateAdministratorName = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const [error, updateNameAdministratorDto] = domain_1.UpdateNameAdministratorDto.create(req.body);
            if (error)
                return res.status(400).json({ error });
            try {
                yield this.authAdministratorRepository.updateAdministratorName(updateNameAdministratorDto);
        ...
        //     }
        // }
        this.updateAdministratorPhone = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const [error, updatePhoneAdministratorDto] = domain_1.UpdatePhoneAdministratorDto.create(req.body);
            if (error)
                return res.status(400).json({ error });
            try {
                yield this.authAdministratorRepository.updateAdministratorPhone(updatePhoneAdministratorDto);
                res.status(201).json({ message: 'Actualizacion exitosa!' });
            }
            catch (error) {
                this.handleError(error, res);
        ...
        this.updateAdministratorEmail = (req, res) => __awaiter(this, void 0, void 0, function* () {
            const [error, updateEmailAdministratorDto] = domain_1.UpdateEmailAdministratorDto.create(req.body);
            if (error)
                return res.status(400).json({ error });
            try {
                yield this.authAdministratorRepository.updateAdministratorEmail(updateEmailAdministratorDto);
                res.status(201).json({ message: 'Actualizacion exitosa!' });
            }
            catch (error) {
                this.handleError(error, res);
